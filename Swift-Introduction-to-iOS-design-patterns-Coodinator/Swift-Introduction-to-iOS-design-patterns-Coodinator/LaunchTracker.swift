...
struct LaunchTracker {
        ...
        static func create(launchType: AppCoordinator.LaunchType) -> Event? {
            ...
            case .userActivity(let activity):
                ...
                case NSUserActivityTypeBrowsingWeb:
                    guard let url = activity.webpageURL else { fatalError("unreachable") }
    ...
    static func track(lauchType: AppCoordinator.LaunchType) {
        guard let event = Event.create(launchType: lauchType) else { return }
        send(event: event)
    }
